...
// Uploads server-generated bytes (e.g. a rendered PDF) directly to storage and
// records an active artifact in one step. Unlike initiateArtifactUpload (which
// hands a signed URL back to a browser), this is for server-side producers that
...
export async function uploadGeneratedArtifact(
  ...
  // tenant/<type>/<timestamp>-<filename> keeps generated files out of the
  // per-artifact versioned tree used by the signed-upload flow.
  const storagePath = `${ctx.tenantId}/${opts.artifactType}/${Date.now()}-${opts.filename}`

  const { error: uploadErr } = await supabase.storage
    .from(STORAGE_BUCKET)
    .upload(storagePath, opts.bytes, { contentType: mimeType, upsert: true })

  if (uploadErr) {
    throw new Error(`uploadGeneratedArtifact upload: ${uploadErr.message}`)
  }
  ...
  const { data: artifact, error: artifactError } = await supabase
    ...
    .insert({
      ...
      artifact_type:   opts.artifactType,
      mime_type:       mimeType,
      file_size_bytes: opts.bytes.byteLength,
      storage_bucket:  STORAGE_BUCKET,
      storage_path:    storagePath,
      status:          'active',
      is_latest:       true,
  ...
  if (artifactError || !artifact) {
    throw new Error(`uploadGeneratedArtifact insert: ${artifactError?.message}`)
  }

  return { artifactId: artifact.id, storagePath }
}
