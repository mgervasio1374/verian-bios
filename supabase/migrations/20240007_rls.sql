...
-- -------------------------------------------------------
CREATE OR REPLACE FUNCTION auth.is_workspace_member(ws_id uuid) RETURNS boolean AS $$
  SELECT EXISTS (
...
CREATE POLICY "role_permissions_read" ON role_permissions
  FOR SELECT USING (
    EXISTS (SELECT 1 FROM roles WHERE id = role_id AND (tenant_id IS NULL OR tenant_id = auth.tenant_id()))
  );

ALTER TABLE memberships ENABLE ROW LEVEL SECURITY;
CREATE POLICY "memberships_own_read" ON memberships
  FOR SELECT USING (user_id = auth.uid() OR tenant_id = auth.tenant_id());
...
-- -------------------------------------------------------
-- CRM TABLES — tenant isolation + workspace membership
-- -------------------------------------------------------
DO $$
DECLARE
  t text;
...
BEGIN
  FOREACH t IN ARRAY tables LOOP
    ...
    EXECUTE format(
      'CREATE POLICY %L ON %I FOR ALL USING (
        tenant_id = auth.tenant_id()
        AND auth.is_workspace_member(workspace_id)
        AND deleted_at IS NULL
      )',
      t || '_rls', t
    );

    -- Allow admin to see deleted records
    EXECUTE format(
      'CREATE POLICY %L ON %I FOR SELECT USING (
        tenant_id = auth.tenant_id()
...
-- activities and conversations don't have deleted_at — fix those
...
CREATE POLICY "approval_requests_assignee_or_admin" ON approval_requests
  FOR ALL USING (
    tenant_id = auth.tenant_id()
    AND (assignee_id = auth.uid() OR auth.is_tenant_admin())
  );
...
CREATE POLICY "prompt_configs_tenant_read" ON prompt_configs
  FOR SELECT USING (tenant_id IS NULL OR tenant_id = auth.tenant_id());
...
CREATE POLICY "dashboard_configs_tenant" ON dashboard_configs
  FOR ALL USING (
    tenant_id = auth.tenant_id()
    AND (is_shared = true OR created_by = auth.uid() OR auth.is_tenant_admin())
  );
...
BEGIN
  FOREACH t IN ARRAY score_tables LOOP
    EXECUTE format('ALTER TABLE %I ENABLE ROW LEVEL SECURITY', t);
    EXECUTE format(
      'CREATE POLICY %L ON %I FOR SELECT USING (tenant_id = auth.tenant_id())',
      t || '_read', t
    );
  END LOOP;
END;
...
CREATE POLICY "recommendation_feedback_own" ON recommendation_feedback
  FOR ALL USING (tenant_id = auth.tenant_id() AND user_id = auth.uid());
