...
export function LeadActivityTimeline({ events }: LeadActivityTimelineProps) {
  if (events.length === 0) {
    return (
      <Card>
        <CardHeader><CardTitle className="text-sm">Workflow Activity</CardTitle></CardHeader>
        <CardContent>
          <p className="text-sm text-muted-foreground">No workflow activity recorded yet for this lead.</p>
        </CardContent>
      </Card>
    )
  }
